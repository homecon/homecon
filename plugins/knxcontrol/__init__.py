...
import numpy as np
import pymysql
import plugins.knxcontrol.mysql
...
class KNXControl:
	...
	def __init__(self, smarthome, mysql_pass=None):
		...
		# test
		self._sh.scheduler.add('testname', self._test, prio=5, cron='* * * *')
		logger.warning(self)
		logger.warning(dir(plugins.knxcontrol.mysql))

	def run(self):
		self.alive = True

		# set the knxcontrol item and initialize measurements
		self.item = self._sh.return_item('knxcontrol')
		self._measurements_init()
	...
	def system_identification(self):
		logger.warning('System identification started')
	...
	def _mysql_init(self):
		""" Function creates mysql tables required for knxcontrol
		"""
	...
	def _measurements_init(self):
		...
		for item in self.item.building:
			item_name = item.id().split(".")[-1]
			id = id + 1
		...
		for item in self.item.ventilation:
			item_name = item.id().split(".")[-1]
			id = id + 1
		...
		for item in self.item.heat.production:
			item_name = item.id().split(".")[-1]
			id = id + 1
		...
		for item in self.item.heat.emission:
			item_name = item.id().split(".")[-1]
			id = id + 1
		...
		for item in self.item.electricity.production:
			item_name = item.id().split(".")[-1]
			id = id + 1
