def _measurements_init(self)
	...
	for item in self._sh.knxcontrol.energy:
	...
	for item in self._sh.knxcontrol.building:
		item_name = item.id().split(".")[-1]
		id = id + 1
	...
	for item in self._sh.knxcontrol.ventilation:
		item_name = item.id().split(".")[-1]
		id = id + 1
	...
	for system in self._sh.knxcontrol.heat.production:
	...
	for system in self._sh.knxcontrol.heat.emission:
		system_name = system.id().split(".")[-1]
		id = id + 1
	...
	for system in self._sh.knxcontrol.electricity.production:
		system_name = system.id().split(".")[-1]
		id = id + 1
