...
#               print ssl.OPENSSL_VERSION
    ...
    def parse_item(self, item):
        acl = self.__acl
        # fixme to allow different permissions for different users, including no permission to read
        if 'visu_acl' in item.conf:
            if item.conf['visu_acl'] in ('yes', 'rw'):
                ...
                acl = 'ro'
        self.visu_items[item.id()] = {'acl': acl, 'item': item, 'read_users':[],'write_users':[]}
        ...
        if not self._user == None:
            if path in self.monitor['item']:
                if self._user['id'] in self.items[path]['read_users']:
                    ...
                    logger.warning('User {} is trying to monitor item {} for which he does not have permission'.format(self._user['username'],path))
        else:
            logger.warning('A unauthenticated user is trying to monitor item {}'.format(path))
