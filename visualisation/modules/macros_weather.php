...
function display_detailed_weather_forecast(){	
	...
	$cloudfactor = '[';
	for($i=0,$i<count($forecast['temperature']);$i++){
		$temperature .= '['.$forecast['timestamp'].','.$forecast['temperature'].'],';
		$cloudfactor .= '['.$forecast['timestamp'].','.$forecast['cloudfactor'].'],';
	}
	$temperature = rtrim($temperature, ',').']';
	$cloudfactor = rtrim($cloudfactor, ',').']';
		...
		echo "
				<div class='weatherforecast'>
					...
					<script>
						...
						weatherchart = new Highcharts.Chart(options);
					</script>";
	...
	for($i=0,$i<count($forecast['temperature']);$i++){			
		echo "
					<div class='table'>
						...
						<p class='small'>".$forecast['precipitation'][$i]." mm/h</p>
						<p class='small'>".$forecast['windspeed'][$i]." m/s   ".$forecast['winddirection'][$i]."&deg;</p>
						<p class='small'>".$forecast['pressure'][$i]." hPa</p>
					</div>";
	}
	echo "
				</div>";
