...
$(document).delegate('[data-widget="basic.value"]', { 
        'update': function(event, response) {
				...
				var rounded = Math.round(response * Math.pow(10,decimals)) / Math.pow(10,decimals)
				var value = parseFloat(rounded).toFixed(decimals);
				
                $('#' + this.id).html( value + '' + $(this).attr('data-unit'));
    }
});                                                
...
// ----- basic.button_plus ----------------------------------------------------------
$(document).delegate('a[data-widget="basic.button_plus"]', { 
        'click': function(event) {
		
				var mod = 10000000;
				if ($(this).attr('data-mod') != ''){
						mod = parseInt($(this).attr('data-mod'));
				}
                if ($(this).attr('data-val') != ''){
				
						var val =  widget.get( $(this).attr('data-item') ) + parseInt($(this).attr('data-val'));
                        if (val > mod-1){
							val = val-mod;
						}
						else if (val < 0){
						...
						io.write($(this).attr('data-item'), val );
				}
        }
});
