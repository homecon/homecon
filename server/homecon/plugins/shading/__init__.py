...
class Shading(core.plugin.Plugin):
    ...
    def initialize(self):
        ...
        logging.debug('Shading plugin Initialized')
    ...
    def auto_position(self):
        ...
        for w in windows:
            ...
            for s in shadings:
                ...
                relativeposition_min_temp = s.calculate_relative_position( s.states['position_min'].value if ( (s.states['override'].value is None or not s.states['override'].value>0) and s.states['auto'].value) else s.states['position_status'].value )
                relativeposition_max_temp = s.calculate_relative_position( s.states['position_max'].value if ( (s.states['override'].value is None or not s.states['override'].value>0) and s.states['auto'].value) else s.states['position_status'].value )
        ...
        for zone in zones:
            ...
            solargain_temp = sum([solargain_temp[w.path] for w in windows])

            # check if repositioning is required / allowed
            c1 = abs(solargain_temp-solargain_set) > solargain_tol
            c2 = (solargain_temp < solargain_tol and sum(relativeposition_old[(w.path,s.path)] if relativeposition_min[(w.path,s.path)]<relativeposition_max[(w.path,s.path)] else 0 for w in windows for s in shadings[w.path] )>1e-3)
            c3 = False
            ...
            if c1 or c2 or c3:
                ...
                data={None:{
                    ...
                    'cost_visibility':{(w.path,): 1.*w.config['cost_visibility'] for w in windows},
                ...
                logging.debug('Recalculated shading positions for zone {}'.format(zone.path))
                ...
                for w in windows:
                    for s in shadings[w.path]:
                        if ((s.states['override'].value is None or not s.states['override'].value>0) and s.states['auto'].value):
                            ...
                            shading.states['position'].set(position,source=self)
    ...
    def listen_state_changed(self,event):
        ...
        if 'component' in state.config:
            ...
            if component.type == 'shading' and ( state == component.states['auto'] or state == component.states['position_min'] or state == component.states['position_max']):
